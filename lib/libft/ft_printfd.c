...
static void	ft_printer(int fd, char c, char *ptr, int *flags)
{
	while (flags[5]-- > 0)
		flags[0] += write(fd, " ", 1);
	if (c != 'c' && c != 's')
	{
		...
		if ((*ptr != '0' && c == 'x' && flags[35]) || (c == 'p' && *ptr))
			flags[0] += write(fd, "0x", 2);
		else if (*ptr != '0' && c == 'X' && flags[35])
			flags[0] += write(fd, "0X", 2);
		while (flags[4]-- > 0)
			flags[0] += write(fd, "0", 1);
	}
	if (c == 's' && !ptr && !(flags[46] && flags[2] < flags[3]))
		flags[0] += write(fd, "(null)", flags[3]);
	else if (c == 'p' && !*ptr)
		flags[0] += write(fd, "(nil)", 5);
	else if (ptr)
		flags[0] += write(fd, ptr, flags[3]);
	while (flags[6]-- > 0)
		flags[0] += write(fd, " ", 1);
}

static void	ft_str_builder(int fd, char c, char *ptr, int *flags)
{
	...
	return (ft_printer(fd, c, ptr, flags));
}
...
int	ft_printfd(const char *str, int fd, ...)
{
	...
	while (*str && fd >= 0)
	{
		if (*str == '%' && *(str - 1) != 92)
		{
			...
			if (*str != '%')
				ft_str_builder(fd, *str, ptr, flags);
			else
				flags[0] += write(fd, str, 1);
			if (*str != 's')
				free(ptr);
		}
		else
